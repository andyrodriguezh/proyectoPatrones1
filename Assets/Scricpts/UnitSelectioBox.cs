...
public class UnitSelectioBox : MonoBehaviour
{
    Camera myCam;
    ...
    private void Update()
    {
        ...
        if (Input.GetMouseButton(0))
        {
            ...
            {
                UnitSelectorManager.Instance.DeselectAll();
                SelectUnits();
            }
    ...
    void SelectUnits()
    {
        foreach (var unit in UnitSelectorManager.Instance.allUnitsList)
        {
            if (selectionBox.Contains(myCam.WorldToScreenPoint(unit.transform.position)))
            {
                UnitSelectorManager.Instance.DragSelect(unit);
            }
        }
    }
}